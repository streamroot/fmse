...
package com.hls {
	...
	internal class HTTPStreamingMP2PESAudio extends HTTPStreamingMP2PESBase
	{
		...
		//aded for WTF
		private var _localOffset:Number = 0;
		...
		override public function processES(pusi:Boolean, packet:ByteArray, offset:Number = 0, flush:Boolean = false): ByteArray {
			if(pusi)
			{
				...
				if(flags == 0x02 || flags == 0x03 )
				{
					...
					var pts:Number =
						...
						((packet.readUnsignedShort() & 0xfffe) >> 1) - 900000; // -10 secs magic offset


					/*if (Math.abs(offset-_localOffset/1000)>1){
							_localOffset = Math.round(pts/90);
					}*/

					_timestamp = Math.round(pts/90)-offset;
			...
			if(flush)
			{
				//throw new Error("Discarding packet")
